...
//#define  LCD_RD_DATA(value)  IORD(LT24_CONTROLLER_0_BASE,0x02)
...
{
        LCD_SetCursor(x,y);
        LCD_WR_REG(0x002C);			//Memory Write
        LCD_WR_DATA(color);
}

/*void LCD_GetImage(DESK_INFO *desk, uint16_t *img, alt_video_display *display)
{
		alt_u16 Xend = DeskInfo->rcPaint.right;
		alt_u16 Yend = DeskInfo->rcPaint.bottom;
		LCD_WR_REG(0x002A);				//CASET=Column Address set
			LCD_WR_DATA(0);				//SC=Start Column
			LCD_WR_DATA(0);
		LCD_WR_DATA(Xend>>8);			//EC=End Column
			LCD_WR_DATA(Xend&0XFF);
	 	LCD_WR_REG(0x002B);				//PASET=Page(row) Address set
			LCD_WR_DATA(0);				//SP=Start Page
			LCD_WR_DATA(0);
			LCD_WR_DATA(Yend>>8);			//EP=End Page
			LCD_WR_DATA(Yend&0XFF);
	 	LCD_WR_REG(0x002E);				//Memory Read

		for(int y=0; y<Yend; y++){
			for(int x=0; x<Xend; x++){
				img[x+y*SCREEN_WIDTH]=LCD_RD_DATA();
			}
		}
}*/
